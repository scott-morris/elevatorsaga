...
| -------- | ----------- | ---------- |
| `settings.DEBUG` | true | `boolean` flag determining whether debugging is turned on |
| `settings.DEBUG_ELEMENTS.ELEVATORS` | `[]` _(empty array)_ | array determining which elevators to track when the `DEBUG` flag is set. To track all, leave the array blank. To track none, populate with `[-1]` |
| `settings.DEBUG_ELEMENTS.FLOORS` | `[]` _(empty array)_ | array determining which floors to track when the `DEBUG` flag is set. To track all, leave the array blank. To track none, populate with `[-1]` |
| `settings.FULL` | `0.7` | The max `loadFactor()` for a given elevator that will stop for a floor that isn't internally pressed |
| `settings.BOTTOM_FLOOR` | `0` | The bottom floor |
| `settings.TOP_FLOOR` | `floors.length - 1` | The top floor, derived by the `floors` array size |
| `settings.NUM_ELEVATORS` | `elevators.length - 1` | The number of elevators, derived by the `elevators` array size |
